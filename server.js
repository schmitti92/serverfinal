...
const RESERVE_MS = 60_000; // keep player slot reserved after disconnect (ms)
...
function restoreRoomState(room){
  try{
    ...
    if(payload && payload.state && typeof payload.state === "object"){
      ...
      // normalize newer fields (backward compatible with older saves)
      room.state.goal = room.state.goal || GOAL;
      if (typeof room.state.winner === "undefined") room.state.winner = null;
      if (!Array.isArray(room.state.legalTargets)) room.state.legalTargets = [];
      return true;
    }
  }catch(_e){}
  return false;
...
function makeRoom(code) {
  return {
    ...
    activeConnIdByToken: new Map(), // sessionToken -> active clientId
    state: null,
...
function initGameState(room) {
  ...
  const barricades = (BOARD.nodes || [])
    .filter(n => n.kind === "board" && n.flags?.run && n.id !== GOAL)
    .map(n => n.id);
...
function computeLegalTargetsForRoll(room, color) {
  const roll = room.state.rolled;
  ...
  for (const pc of room.state.pieces) {
    ...
    if (pc.posKind === "house") {
      const remaining = roll - 1;
      if (remaining === 0) targetsMap = new Map([[startField, [startField]]]);
      else targetsMap = computeAllTargets(room, startField, remaining, color, pc.id);
    } else if (pc.posKind === "board") {
      targetsMap = computeAllTargets(room, pc.nodeId, roll, color, pc.id);
    }
    out.push({ pieceId: pc.id, targets: Array.from(targetsMap.keys()) });
  }
  return out;
}
...
function requireTurn(room, clientId, ws) {
  ...
  if (room.state.phase === "game_over" || room.state.winner) { send(ws, { type: "error", code: "GAME_OVER", message: "Spiel ist vorbei. Bitte Reset." }); return false; }
...
wss.on("connection", (ws) => {
  ...
  ws.on("message", (buf) => {
    ...
    const roomCur = c.room ? rooms.get(c.room) : null;
    // Drop messages from stale / duplicate connections for the same sessionToken
    if (roomCur && c.sessionToken && roomCur.activeConnIdByToken?.get(c.sessionToken) && roomCur.activeConnIdByToken.get(c.sessionToken) !== clientId) {
      // only allow join/ping from stale connections
      if (msg.type !== "join" && msg.type !== "ping") return;
    }
...
if (!color) {
  // keep disconnected players reserved for a while to allow quick reconnect
  // (otherwise the reconnecting player loses their color if someone else joins)
  for (const p of Array.from(room.players.values())) {
    if (p.color && !isConnectedPlayer(p)) {
      const age = Date.now() - (p.lastSeen || 0);
      if (age > RESERVE_MS) room.players.delete(p.id);
    }
  }
...
room.players.set(clientId, { id: clientId, name, color, isHost, sessionToken, lastSeen: Date.now() });
      ...
      // mark this connection as the active one for this token (drop old duplicates)
      if (sessionToken) room.activeConnIdByToken.set(sessionToken, clientId);
      const _wasPaused = !!room.state?.paused;
      resumeIfReady(room);
      if (room.state && _wasPaused && !room.state.paused) {
      ...
      send(ws, { type: "assign", clientId, sessionToken, color, isHost });
    ...
    if (msg.type === "claim_color") {
      ...
      // resume if it was that player's turn
      if (room.state && room.state.paused && room.state.turnColor === targetColor) {
        room.state.paused = false;
      }
  ...
  persistRoomState(room);
    ...
    if (msg.type === "roll_request") {
      ...
      // compute legal targets for UI (optional but helps the client highlight moves)
      room.state.legalTargets = computeLegalTargetsForRoll(room, room.state.turnColor);
      persistRoomState(room);
      // Send both 'rolled' (new client) and 'roll' (legacy) for compatibility
      broadcast(room, { type: "rolled", rolled: v, value: v, legalTargets: room.state.legalTargets, state: room.state });
...
if (msg.type === "move_request") {
      ...
      // WIN: only one piece has to reach the goal
      if (GOAL && landed === GOAL) {
        room.state.winner = pc.color;
        room.state.phase = "game_over";
        room.state.rolled = null;
        room.state.legalTargets = [];
        room.lastRollWasSix = false;
        persistRoomState(room);
        broadcast(room, {
          type: "move",
          action: { pieceId: pc.id, path: res.path, pickedBarricade: false, kickedPieces: kicked, reachedGoal: true },
          state: room.state
        });
        broadcast(room, { type: "room_update", players: currentPlayersList(room), canStart: canStart(room) });
        return;
  ...
  ws.on("close", () => {
    ...
    if (roomCode) {
      ...
      if (room) {
        ...
        // remove active connection mapping for this token (only on close)
        if (c.sessionToken && room.activeConnIdByToken?.get(c.sessionToken) === clientId) {
          try { room.activeConnIdByToken.delete(c.sessionToken); } catch(_e) {}
        }
