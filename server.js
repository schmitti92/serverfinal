...
function canStart(room) {
  // Wichtig: Wenn ein Spiel bereits läuft, darf "canStart" NICHT mehr true werden,
  // sonst triggert der Host-Client beim Reconnect erneut "start" (Reset-Bug).
...
wss.on("connection", (ws) => {
  ...
  ws.on("message", (buf) => {
    ...
    if (msg.type === "join") {
      ...
      // Reconnect-Fix: wenn ein Spieler wieder da ist, Spiel wieder freigeben
    ...
    if (msg.type === "start") {
      ...
      // Reconnect-Schutz: Wenn das Spiel schon läuft, NICHT neu initialisieren.
      // (Sonst: Host reconnect -> room_update(canStart=true) -> start -> initGameState -> Reset)
      if (room.state?.started) {
        send(ws, { type: "snapshot", state: room.state });
        // room_update hinterher schadet nicht und hält UI synchron
        send(ws, { type: "room_update", players: currentPlayersList(room), canStart: canStart(room) });
        return;
      }
    ...
    // ---------- SAVE / LOAD (Host) ----------
    // Speichern als Datei passiert clientseitig (Download). Server liefert nur den autoritativen Zustand.
    if (msg.type === "save_request") {
      if (!room.state?.started) {
        send(ws, { type: "error", code: "NO_GAME", message: "Kein laufendes Spiel zum Speichern." });
        return;
      }
      // Alle Infos, die fürs Fortsetzen nötig sind (nicht nur room.state)
      send(ws, {
        type: "save_data",
        save: {
          v: 1,
          ...
          room: room.code,
          state: room.state,
          lastRollWasSix: !!room.lastRollWasSix,
          carryingByColor: room.carryingByColor || { red: false, blue: false }
        }
      });
      return;
    }
    ...
    if (msg.type === "load_save") {
      const me = room.players.get(clientId);
      if (!me?.isHost) { send(ws, { type: "error", code: "NOT_HOST", message: "Nur Host kann laden" }); return; }

      const save = msg.save;
      if (!save || typeof save !== "object") {
        send(ws, { type: "error", code: "BAD_SAVE", message: "Save-Datei ungültig." });
        return;
      }
      const st = save.state;
      if (!st || typeof st !== "object" || !Array.isArray(st.pieces) || !Array.isArray(st.barricades) || !st.turnColor) {
        send(ws, { type: "error", code: "BAD_SAVE", message: "Save enthält keinen gültigen Spielstand." });
        return;
      }
      ...
      room.lastRollWasSix = !!save.lastRollWasSix;
      room.carryingByColor = save.carryingByColor || { red: false, blue: false };

      console.log(`[load] room=${room.code} by=host ts=${save.ts || "?"}`);
      broadcast(room, { type: "snapshot", state: room.state });
      broadcast(room, { type: "room_update", players: currentPlayersList(room), canStart: canStart(room) });
      broadcast(room, { type: "load_done" });
      return;
    }
