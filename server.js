...
// ---------- Server-side move animation (Weg 1) ----------
// Advance field-by-field on the server so both clients see the SAME step animation.
// During animation we lock the room using phase="moving".
function scheduleAnimatedMove(room, plan) {
  // Cancel a previous animation if any (safety)
  if (room._moveTimer) {
    try { clearTimeout(room._moveTimer); } catch (_e) {}
  ...
  // Inform both clients a move is starting (for visuals)
  broadcast(room, {
    type: "move_begin",
    action: {
  ...
  const tick = () => {
    const p = room._movePlan;
    if (!p || !room.state || !room.state.started) return;

    // Move step
    if (p.i < p.path.length) {
      const nodeId = p.path[p.i];
      const pc = getPiece(room, p.pieceId);
      if (pc) {
        pc.posKind = "board";
        pc.nodeId = nodeId;
      }
      ...
      broadcast(room, {
        type: "move_step",
        action: { pieceId: p.pieceId, nodeId, i: p.i, n: p.path.length },
        state: room.state
      });

      p.i += 1;
      room._moveTimer = setTimeout(tick, p.stepMs);
      return;
    }
  ...
  room._moveTimer = setTimeout(tick, 0);
}
...
wss.on("connection", (ws) => {
  ...
  ws.on("message", (buf) => {
    ...
    if (msg.type === "join") {
      ...
      // host assignment (first host only)
      ...
      if (!color) {
        if (used.length >= 2) { send(ws, { type: "error", code: "ROOM_FULL", message: "Raum ist voll (max. 2 Spieler)." }); return; }
    ...
    if (msg.type === "roll_request") {
      ...
      if (room.state.phase === "moving") {
        send(ws, { type: "error", code: "MOVING", message: "Zug läuft noch…" });
        return;
      }
    ...
    if (msg.type === "move_request") {
      ...
      const path = res.path.map(String);
      const landed = String(path[path.length - 1]);

      // Determine what will happen at the END of the move
      const willPick = room.state.barricades.includes(landed);

      const stepMs = 260; // visually nice, not too slow

      scheduleAnimatedMove(room, {
        pieceId: pc.id,
        path,
        i: 0,
        stepMs,
        finalize: () => {
          ...
          if (willPick) {
            ...
            if (idx >= 0) room.state.barricades.splice(idx, 1);
            picked = true;
            room.carryingByColor[pc.color] = true;
            room.state.phase = "place_barricade";
          } else {
            // next turn (or extra roll)
            if (room.lastRollWasSix) room.state.turnColor = pc.color;
            else room.state.turnColor = otherColor(pc.color);
            room.state.phase = "need_roll";
            room.state.rolled = null;
          }

          console.log(`[move] room=${room.code} color=${pc.color} piece=${pc.id} to=${landed} picked=${picked} (animated)`);
          broadcast(room, {
            type: "move_end",
            action: { pieceId: pc.id, path, pickedBarricade: picked, kickedPieces: kicked },
            state: room.state
          });
          broadcast(room, { type: "room_update", players: currentPlayersList(room), canStart: canStart(room) });
        }
