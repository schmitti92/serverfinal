...
room.players.set(clientId, { id: clientId, name, color, isHost, sessionToken, lastSeen: Date.now() });
      ...
      // Auto-unpause aktiv: Sobald wieder 2 Spieler verbunden sind, geht es automatisch weiter
...
if (msg.type === "move_request") {
      ...
      if (landed === GOAL) {
        ...
        const goals = room.state.pieces.filter(p => p.color === pc.color && p.posKind === "goal").length;
        if (goals >= 1) {
          room.state.winner = pc.color;
          room.state.phase = "game_over";
          room.state.rolled = null;
          // clear carry flag just in case
          room.carryingByColor[pc.color] = false;
        }
      }
      ...
      if (idx >= 0) {
        ...
        if (!room.state.winner) room.state.phase = "need_roll";
      }

      // if no barricade placement needed:
      if (!picked && !room.state.winner) {
