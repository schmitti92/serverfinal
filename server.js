...
wss.on("connection", (ws) => {
  ...
  ws.on("message", (buf) => {
    ...
    if (msg.type === "start") {
      ...
      if (room.animTimer) { try { clearTimeout(room.animTimer); } catch(_e){} room.animTimer = null; }
      initGameState(room);
...
// ---- Animation lock: prevent next roll until the move animation can finish on clients ----
const nextPhase = room.state.phase;
room.state.phase = "anim";

// Duration: field-by-field pacing (visual), capped so it never feels stuck.
const steps = Math.max(1, (res.path?.length || 2) - 1);
const animDelayMs = Math.min(3200, 260 + steps * 520);

// Clear any previous timer (safety)
if (room.animTimer) {
  try { clearTimeout(room.animTimer); } catch (_e) {}
  room.animTimer = null;
}

console.log(`[move] room=${room.code} color=${pc.color} piece=${pc.id} to=${pc.nodeId} picked=${picked} anim=${animDelayMs}ms`);
broadcast(room, {
  type: "move",
...
room.animTimer = setTimeout(() => {
  // Only unlock if still in anim (could have been reset)
  if (room.state && room.state.phase === "anim") {
    room.state.phase = nextPhase;
    broadcast(room, { type: "snapshot", state: room.state, players: [...room.players.values()] });
  }
  room.animTimer = null;
}, animDelayMs);
