...
wss.on("connection", (ws) => {
  ...
  ws.on("message", (buf) => {
    ...
    if (msg.type === "join") {
      ...
      // ✅ Unpause if the active (turn) player is connected again (fix: can't roll after reconnect)
      if (room.state && room.state.paused) {
        const turn = room.state.turnColor;
        const turnConnected = Array.from(room.players.values()).some(p => p.color === turn && isConnectedPlayer(p));
        if (turnConnected) room.state.paused = false;
      }
