...
room.players.set(clientId, { id: clientId, name, color, isHost, sessionToken, lastSeen: Date.now() });
    ...
    if (msg.type === "claim_color") {
      ...
      // Wichtig: room.state bleibt unverändert (Figuren sind farbgebunden)
      broadcast(room, { type: "room_update", players: currentPlayersList(room), canStart: canStart(room) });
      if (room.state) {
        persistRoomState(room);
    ...
    // ---------- SWAP COLORS (Host only) ----------
    // Notfall: Rot ↔ Blau tauschen (nur Zuordnung der Spieler, Spielstand bleibt unverändert)
    if (msg.type === "swap_colors") {
      const me = room.players.get(clientId);
      if (!me?.isHost) { send(ws, { type: "error", code: "NOT_HOST", message: "Nur Host" }); return; }

      // finde aktuelle Halter von Rot/Blau (online oder offline placeholder egal)
      let redP = null, blueP = null;
      for (const p of room.players.values()) {
        if (p.color === "red" && !redP) redP = p;
        if (p.color === "blue" && !blueP) blueP = p;
      }

      if (!redP || !blueP) {
        send(ws, { type: "error", code: "SWAP_FAIL", message: "Zum Tauschen müssen Rot und Blau belegt sein" });
        return;
      }
      ...
      broadcast(room, { type: "room_update", players: currentPlayersList(room), canStart: canStart(room) });
      // state bleibt 1:1, aber snapshot schicken damit Clients sofort sauber rendern
      if (room.state) {
        persistRoomState(room);
        broadcast(room, { type: "snapshot", state: room.state });
      }
