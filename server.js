...
wss.on("connection", (ws) => {
  ...
  ws.on("message", (buf) => {
    ...
    if (msg.type === "start") {
      ...
      // ✅ Safety: nicht mitten im Spiel neu starten (verhindert "Reset" durch doppelte start-Messages)
      if (room.state && room.state.started) {
        send(ws, { type: "error", code: "ALREADY_STARTED", message: "Spiel läuft bereits – benutze Reset für Neustart" });
        return;
      }
